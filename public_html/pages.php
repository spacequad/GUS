...
if ((file_exists(GUS_cachefile())) && ($today != $day . $month . $year)) {
    ...
    $T->set_var('data',$LANG_GUS00['page']);
    ...
    for ($i=0; $i<$nrows; $i++)
    {
    	...
    	$T->set_var( 'colclass', 'col_left' );
        $T->set_var('data','<a href="' . $_CONF['site_url'] . '/gus/page.php?year=' . $year . '&amp;month=' . $month . '&amp;day=' . $day . '&amp;visited_page=' . $A['page'] . '">' . $A['page'] . '</a>');
        $T->parse('CBlock','COLUMN',false);
    ...
    $T->set_var( 'colclass', 'col_left' );
    $T->set_var('data',$LANG_GUS00['total']);
    $T->parse('CBlock','COLUMN',false);

    $T->set_var( 'colclass', 'col_right' );
    $T->set_var('data',$pageviews);
    $T->parse('CBlock','COLUMN',true);
    ...
    if ($pageviews == 0 || $visitors == 0 ) {
        ...
        $avgViews = $pageviews/$visitors;
    }

    $T->set_var('summary',"$pageviews {$LANG_GUS00['total']} {$LANG_GUS00['page_title']}<br>$visitors {$LANG_GUS00['unique_visitors']}<br>" . sprintf("%01.2f",$avgViews) . " {$LANG_GUS00['views']}.<br>");

    if ( $day != '' )
        $title = Date( 'l, j F, Y - ', mktime( 0, 0, 0, $month, $day, $year ) ) . $LANG_GUS00['views_per_page'];
    else
        $title = Date( 'F Y - ', mktime( 0, 0, 0, $month, 1, $year ) ) . $LANG_GUS00['views_per_page'];
