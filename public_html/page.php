...
// +--------------------------------------------------------------------------+
// | page.php                                                                 |
// | Displays stats for a specific page                                       |
// +--------------------------------------------------------------------------+
// | $Id::                                                                   $|
// +--------------------------------------------------------------------------+
...
$ip = isset( $_GET['ip'] ) ? COM_applyFilter($_GET['ip']) : '';

$uid = isset( $_GET['uid'] ) ? COM_applyFilter($_GET['uid']) : '';
settype( $uid, 'integer' );

$visited_page = isset( $_GET['visited_page'] ) ? COM_applyFilter($_GET['visited_page']) : '';

$anon = isset( $_GET['anon'] ) ? COM_applyFilter($_GET['anon']) : '';
settype( $anon, 'integer' );
...
if ((file_exists(GUS_cachefile())) && ($today != $day . $month . $year)) {
    ...
    $title = $date_formatted . $ip_str . $visited_page . '&nbsp;' . $LANG_GUS00['page_views'];
